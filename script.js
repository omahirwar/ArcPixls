...
const ownerWalletList = document.getElementById('owner-wallet-list');
const ownerWalletCount = document.getElementById('owner-wallet-count');
const copyWalletsBtn = document.getElementById('copy-wallets-btn');

let currentWalletsList = [];

async function fetchOwnerSubmissions() {
  if (!ownerWalletList || !ownerWalletCount) return;
  ownerWalletList.innerHTML = '<div>Loading submissions...</div>';
  try {
    const res = await fetch('/api/whitelist');
    const data = await res.json();
    currentWalletsList = data.wallets || [];
    ownerWalletCount.textContent = currentWalletsList.length;

    if (currentWalletsList.length === 0) {
      ownerWalletList.innerHTML = '<div style="color: #71717a; text-align: center; padding: 12px;">No wallets submitted yet.</div>';
      return;
    }

    ownerWalletList.innerHTML = currentWalletsList.map((item, idx) => {
      const timeStr = new Date(item.submittedAt).toLocaleString();
      return `<div class="wallet-item-row">
        <span><strong>#${idx + 1}</strong> ${item.wallet}</span>
        <span class="wallet-time">${timeStr}</span>
      </div>`;
    }).join('');
  } catch (err) {
    ownerWalletList.innerHTML = '<div style="color: #ef4444;">Failed to load submissions.</div>';
  }
}

if (ownerPanelBtn && ownerModal) {
...
if (copyWalletsBtn) {
  copyWalletsBtn.addEventListener('click', () => {
    if (!currentWalletsList.length) return;
    const textToCopy = currentWalletsList.map(w => w.wallet).join('\n');
    navigator.clipboard.writeText(textToCopy).then(() => {
      const orig = copyWalletsBtn.textContent;
      copyWalletsBtn.textContent = '✅ Copied!';
      setTimeout(() => { copyWalletsBtn.textContent = orig; }, 2000);
    });
  });
}
